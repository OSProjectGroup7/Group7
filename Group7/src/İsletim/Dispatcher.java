...
public class Dispatcher {
	...
	public void Start_Dispatcher() {
		...
		while (used_items_count != size || !fpl.FPL_isEmpty() || !spl.SPL_isEmpty() || !rr.RR_isEmpty()) { // tüm kuyruklar boşaldığında bitecek.
	...
	public void TimeOut_Scanner(int gecenZaman) { //20 saniye olduğunda kuyruktan çıkarma fonksiyonu.

		String text = "";

		for (int i = 0; i < rtq.getRealTimeQueue().size(); i++) { //Real time için
		...
		for (int i = 0; i < fpl.getFirstPriorityList().size(); i++) { //FPL için
		...
		for (int i = 0; i < spl.getSecondPriorityList().size(); i++) { //spl için
		...
		for (int i = 0; i < rr.getRoundRobinList().size(); i++) { //round robin için
